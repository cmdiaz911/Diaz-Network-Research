# -*- coding: utf-8 -*-
"""
Created on Sat Jan 14 13:49:53 2017
...
# FIX THE MODIFICATION STEPS AND THE BID STEPS. WHY ARE THERE NO SUBTRACTIONS IN THE ESTIMATES 
# FIX THE NAMING FOR THE NODES THERE NEEDS TO BE A BETTER WAY OF DOING THIS MAYBE MAP STRINGS TO A DICTIONARY FOR THE NODES 
# FIX EVERYTHING ELSE THAT LOOKS WEIRD, THE ORIGINAL PROBLEM WAS THE INDEXING, LOOK FOR MORE ISSUES WITH THIS 
# CODE A BETTER WAY TO FIND RESULTS 
...
class UAVnet(nx.DiGraph): # DiGraph is a class in networkx, this makes us inherit the Directed Graph class from networkx 
	...
	def CreateMarkovDictionary(self):
		...
		for node in self.nodes(): # create add to the dictionary for every node
			self.Markov_Probabilities['Total_Lists'][node] = [0 for x in range(len(self.nodes()))] # create the list of probabilitys for total probability
			self.Markov_Probabilities['Neighbor_Lists'][node] = {} # create the neighbor list subdictionary
	...
	def AddAuxNode(self, xsource_node, xtarget_node): # target and source node have to be objects
		...
		if self.is_markov_graph: # if the graph is a markove graph
			for out_neigh in xsource_node.outgoing_ties: # for the outdirected ties
				self.add_node("%s,source,aux" % out_neigh) # 1012 is node 1 to node 12 , outdirected ties
			...
			for in_neigh in xtarget_node.incoming_ties: # this creates the indirected aux nodes
				
				self.add_node("%s,target,aux" % in_neigh) # create the aux node for the indirected neighbor
			...
			for node in self.nodes(): # create the absorption nodes for the message dyinig
				self.add_node("%s,absorb" %node) # create the absorption node
				self.add_edge(node ,"%s,absorb" % node) # creae the edge to the absorption node
	...
	def Update_MC_Matrix(self, xsource_node, xtarget_node, xUAVnet):
		...
		if self.is_markov_graph: # if the graph being solved is the markov graph it will pass this test 
			...
			matrix_T = matrix_A[0:no_nodes_original, 0:(no_nodes_original + number_of_aux_source_nodes + number_of_aux_target_nodes)] # the matrix that needs to get the probability of random walk placed in 
			matrix_abs = matrix_A[0: no_nodes_original, (no_nodes_original + number_of_aux_source_nodes + number_of_aux_target_nodes):(2*no_nodes_original + number_of_aux_source_nodes + number_of_aux_target_nodes)] # the self node ties 
			
			########## YOU ARE THE CURRENT PROBLEM##########
			for node in range(len(matrix_T)): # input the probability of a message being passed to any of the outgoing neighbors random walk 
			...
			###############################################
			for node in range(len(matrix_abs)): # then input the proabability of messages dying 
				for other_node in range(no_nodes_original):
					if matrix_abs[node, other_node] > 0:
						matrix_abs[node, other_node] = self.p
			
			
			Matrix_t = matrix_A[0: no_nodes_original + number_of_aux_source_nodes, 0:no_nodes_original+number_of_aux_source_nodes] # this is the actual transient matrix 
			Identity = np.identity(no_nodes_original + number_of_aux_source_nodes) # the identity matrix that is needed to solve the probability
			matrix_B = copy.deepcopy(matrix_A)
			matrix_solver = Identity - Matrix_t # the matrix that needs to be inverted 
			nodes_that_are_solving = [x for x in node_order_list[no_nodes_original + number_of_aux_source_nodes: no_nodes_original + number_of_aux_source_nodes + number_of_aux_target_nodes]] # the neighbors of the target nodes, the aux nodes for the target node
			#print Matrix_t
			
			for indirected_neighbor in range(number_of_aux_target_nodes): # run it over all indirected neigbors of the target node 
				node_being_grabbed = node_order_list[no_nodes_original + number_of_aux_source_nodes + indirected_neighbor]
				
				split_to_original = node_being_grabbed.split(",")
				neighbor_node = int(split_to_original[0])
	
				solved_probs = matrix_solver.I*matrix_B[0:no_nodes_original + number_of_aux_source_nodes, no_nodes_original + number_of_aux_source_nodes + indirected_neighbor] # the list of the total solved probabilities 
				...
				for node in range(number_of_aux_source_nodes): # over the number of aux target nodes 
					solver_tracker.append((1/float(number_of_aux_source_nodes))*solved_probs[no_nodes_original+node,0]) # save the correct probability 
				
				total_prob_to_target = sum(solver_tracker) # use the sum as the probability saved 
				
				xUAVnet.Markov_Probabilities['Neighbor_Lists'][xtarget_node.name][neighbor_node][xsource_node.name] = total_prob_to_target # save it to the original graphs dictionary 
			...
			for node in nodes_that_are_solving: # for the nodes that we solved for 
				...
				total_total_tracker.append(xUAVnet.Markov_Probabilities['Neighbor_Lists'][xtarget_node.name][in_neighbor][xsource_node.name]) # append the probability to the intermediate list 
	...
	def ModifyGraph(self):
		...
		for decision in self.decision_list: # loop through the decision list 
			...
			elif decision[2] == -1: # if the node is deleteing a tie 
...
class UAV(object):
	...
	def CreateAttributes(self, xUAVnet):
		...
		for edge in xUAVnet.edges():
			if edge[0] == self.name: # if the tie is first on the list its outgoing
				self.outgoing_ties.append(edge[1]) # append the node to the list of outgoing neibors
			elif edge[1] == self.name: # if the tie is second on the tuple its incoming
				self.incoming_ties.append(edge[0]) # append the node to the list of indirected neighbors
		
		if len(self.incoming_ties) > 0: # if the number of incoming ties is >0
			for node in self.incoming_ties: # loop over the indirected ties
				self.prob_list_neighbors[node] = [0 for x in range(len(xUAVnet.nodes()))] # create the message from neighbors dictionary
	...
	def UpdateBetaOwn(self):
		for source in range(len(self.prob_list_total)):# update the beta own
			self.Beta_own[source] = 1/math.exp(-(1-(self.prob_list_total[source]))) # beta own for ever source is equal to the negative exponent of the messages received from the source
		...
		if len(self.outgoing_ties)>0:
			...
			for neighbor in Neighbor_object_list: # for the neighbor objects
				for source in range(len(self.prob_list_total)): # for all the sources 
					total_list[source] += neighbor.Beta_own[source]# add their beta own values to the intermediate list
	...
	def UpdateBetaOverall(self):# seperate but doesnt need to be from update beta neighbors
	...
	def CalculateObjective(self):
		...
		cost_tracker = 0# set an intermeddiate tracker that tracks the value of the cost 
		for neighbor in self.incoming_objects: # for each neighor that the agent is pulling from
	...
	def EstimateAdditions(self):
		...
		if len(self.potential_objects) > 0:
			for node in self.potential_objects:
				...
				for index in range(len(self.prob_list_total)): # loop over the message indeces 
				...
				if not self.too_isolated: # the node will lie if it is too isolated
					self.addition_estimates.append([self.objective_value + delta_obj, node.name, 1]) # if it isnt isolated just report normal 
				else:
	...
	def EstimateSubtraction(self):
		self.subtraction_estimates = []
		if not self.too_isolated: # if current node is not too isolateed
			#print self.incoming_objects[0].name,self.name
			for node in self.incoming_objects: # look at currently connected nodes
				
				neighbor_prob_list = self.prob_list_neighbors[node.name] # extrct the message list from the neighbor node
				prob_list_without_neighbor = [0]*len(self.prob_list_total) # create list to use for updating information
				for index in range(len(self.prob_list_total)):# loop over the list indeces
					...
					prob_list_without_neighbor[index] = prob_list_without_neighbor[index] * self.Beta_Overall[index] # multipy by the weights
	...
	def CreateDecisionList(self):
		self.decision_list = [] 
		self.decision_list.append([self.objective_value,self.name,0]) # th do nothing decision is added to the decision list
	...
	def FinalDecisionList(self, xUAVnet):
		self.final_list_of_decisions = None
		self.final_list_of_decisions = sorted(self.decision_list, key=itemgetter(0), reverse = True)# sort the decisions by their effect on the objective 
	...
	def __init__(self, xpossible_edge_list, xfinal_edge_list, xnumber_of_nodes, xnumber_of_rounds,xgraph_name, xgamma, xscaling_factor, xefficiency_scale):
		...
		for node in range(self.number_of_nodes): # loop through all nodes
			for target in range(self.number_of_nodes): # loop through all nodes
				if target in shortest_path_dicts[node] and target != node:# if there is a path from the target to the source 
					distance_list.append(1/float(len(shortest_path_dicts[node][target])-1)) # save the inverse distance to that target
				elif target != node and target not in shortest_path_dicts[node]: # if there is no path save o
					distance_list.append(0)
					
		fully_connected_graph = UAVnet() # save th fully connetcted graph
		...
		fully_connected_path_dicts = nx.shortest_path(fully_connected_graph) # same procedure as above, saves the optimal aver inverse distance 
		...
		self.fully_connected_inverse_distance = float(sum(fully_connected_distance_list))/float(len(fully_connected_distance_list)) # save the average inverse distance 
		self.average_inverse_distance = float(float(sum(distance_list))/float(len(distance_list))) # save the instance average inverse distance
		self.ratio_of_edges = float(float(len(self.final_edge_list))/float(len(self.possible_edge_list))) # save the ratio of edges 
