...
def BOYBQA_Optimizer(xinit_parameters, xlower_bounds, xupper_bounds,xdx, xnode_sizes_list, xmax_eval = 500, xftol = .001):
	...
	node_index = 2 # starts at 10 when 0, 15 when 1 etc 
	optimimum_values = [] 
	global count
	global start_time 
	for node_num in xnode_sizes_list:
		...
		def optimization_function(parameters, grad):
			...
			for file_name in file_list:
				...
				model_results = model.runsimulation(node_num, parameters[0], parameters[1], xtest_list, parameters[2], file_name, seed, 2.5)[0] # to get the result list from the procedure method
			...
			save_algorithm_evaluations(node_num, count, average_inv_distance, average_edge_count, average_edge_ratio, objective_value, seed, parameters, efficiency_scale)
...
def save_algorithm_evaluations(xnode_num, xcount, xaverage_inv_distance, xaverage_edge_count, xaverage_edge_ratio, xobjective_value, xseed, xparameters, xefficiency_scale, xrun = 1):
...
initial_parameters = np.array([.5, .75, 15])
