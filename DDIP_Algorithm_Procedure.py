...
def runsimulation(xnode_number, xgamma, xscaling_factor,xfeasible_edge_set, xnumber_of_periods, xgraph_name, xedge_number = 0):
	...
	if xedge_number != 0: # if there is an input for the number of starting edges on the initial graph
		...
		for node in UAVnetwork.node_objs: # make sure that the nodes communicate their choices for the seed 
			node.UnisolateUAV(UAVnetwork) # unisolate any nodes that have no indirected communication network 
	...
	result = FinalResults(xfeasible_edge_set,UAVnetwork.edges(),len(UAVnetwork.nodes()),xnumber_of_periods, xgraph_name,xgamma, xscaling_factor)
	return [[result.ratio_of_edges, result.average_inverse_distance, 1/result.edge_efficiency,result.edge_number],result]
