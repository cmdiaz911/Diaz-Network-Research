...
for index in range(0,3):
	test_gamma = nelder_mead_gammas[index] # what tdo I make this 
	test_scaling_factor = nelder_mead_scaling[index] # not sure if this should be large or small, will have to test the model and print choices until this makes sense
	for file_name in sample_list_by_node_number[test_node_index]:
		...
		for i in xrange(tests_per_sample):
			...
			list_of_results.append(runsimulation(node_number_list[test_node_index],test_gamma,test_scaling_factor,test_edge_set,test_periods, test_file)[1])
